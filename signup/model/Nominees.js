...
nomineeSchema.statics.findByEmailAndPassword = function(email, password) {
  var userObj = null;
...
nomineeSchema.pre("remove", function(next) {
...
var Nominee
try {
  Nominee = mongoose.model('nominee')
} catch (error) {
  Nominee = mongoose.model('nominee', nomineeSchema)
}
